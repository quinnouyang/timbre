...
parser.add_argument('--regularization', type=str, default='l2', choices=['l2', 'prior', 'student', 'gaussian'],
...
parser.add_argument('--target_dims', type=str, default='all', choices=['min', 'max', 'all'],
...
audioSet.classes['pitch'] = {'A': 0, 'A#': 1, 'B': 2, 'C': 3, 'C#': 4, 'D': 5, 'D#': 6, 'E': 7, 'F': 8, 'F#': 9,
                             'G': 10, 'G#': 11, '_length': 12}
...
if args.load is None:
    ...
    hidden_params = [{'dim': args.hidden_dims, 'nlayers': args.hidden_layers, 'dropout': None, 'residual': False,
                      'batch_norm': True}] * 3
...
if args.twofold and args.load is None:
    ...
    train_options = {'epochs': args.epochs, 'save_epochs': args.save_epochs, 'name': '%s-solo' % args.name,
                     'results_folder': args.saveroot + '/' + args.name + '-solo'}
    train_model(audioSet, vae, elbo_loss, task=args.conditioning, options=train_options, plot_options=plot_options,
...
if args.twofold:
    train_options = {'epochs': args.twofold_epochs, 'save_epochs': args.save_epochs, 'name': '%s-reg' % args.name,
                     'results_folder': args.saveroot + '/' + args.name + '-reg'}
else:
    train_options = {'epochs': args.epochs, 'save_epochs': args.save_epochs, 'name': args.name,
                     'results_folder': args.saveroot + '/' + args.name}
train_model(audioSet, vae, loss, task=args.conditioning, loss_task='instrument', options=train_options,
            plot_options=plot_options, save_with={'sh_args': args})
