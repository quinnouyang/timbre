...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, options):
        ...
        for t in range(len(self.tasks)):
            self.taskCallback[t] = (options.get("taskCallback") and options["taskCallback"][t]) or metadataCallbacks[
                self.tasks[t]] or metadataCallbacks["default"] or []
            self.metadataFiles[t] = (options.get("metadataFiles") and options["metadataFiles"][
                t]) or self.metadataDirectory + '/' + self.tasks[
                                        t] + '/metadata.txt' or self.metadataDirectory + '/metadata.txt'
        ...
        for line in fileCheck:
            if line[0] != "#":
                ...
                if (self.checkIntegrity) and (testFileID is None):
                    if (self.verbose):
                        print(
                            '[Dataset][List file] Warning loading ' + inputFile + ' - File ' + self.dataPrefix + audioPath + ' does not exists ! (Removed from list)')
                else:
                    if (testFileID):
                        testFileID.close()
            ...
            if line[0] != "#" and len(line) > 1:
                ...
                if (audioPath is not None):
                    ...
                    if (len(metaList) - 1 < curHash):
                        metaList.append("");
                    metaList[curHash], classList = callback(metaPath, classList,
    ...
    def createBatches(self, partition, batchSize, balancedTask):
        ...
        (#what if I could put myself to nil
        """
        finalIDs = {};
        if balancedTask:
