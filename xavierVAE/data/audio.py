...
class DatasetAudio(generic.Dataset):
    ...
    def computeTransforms(self, idList, options, padding=False):
        ...
        self.analysisDirectory = options.get(
            'analysisDirectory') or self.analysisDirectory or self.dataPrefix + '/analysis'
        ...
        if (idList is None):
            ...
            if padding:
                indices = np.pad(indices, (0, self.importBatchSize - (len(self.files) % self.importBatchSize)),
...
def makeAnalysisFiles(curBatch, options):
    ...
    if (options["backend"] == 'matlab'):
        ...
        for it in range(len(options["transformType"])):
            transformString = transformString + '\'' + options["transformType"][it] + (
                        (it < (len(options["transformType"]) - 1)) and '\',' or '\'}')
            finalCommand = finalCommand + ' "transformType=' + transformString + '; oldRoot = \'' + options[
                "dataDirectory"] + '\'; newRoot = \'' + options["analysisDirectory"] + '\'';
    ...
    for i in range(len(curBatch)):
        curFile = curBatch[i]
        curAnalysisFiles[i] = \
        os.path.splitext(curFile.replace(du.esc(options["dataDirectory"]), options["analysisDirectory"]))[0] + '.npy'
        try:
    ...
    if (len(audioList) > 0):
        ...
        if (options['backend'] == 'matlab'):
            ...
            for f in range(len(audioList)):
                ...
                unprocessedString = unprocessedString + '\'' + audioList[f] + (
            ...
            for f in range(len(audioList)):
                curFile = \
                os.path.splitext(audioList[f].replace(du.esc(options["dataDirectory"]), options["analysisDirectory"]))[
                    0]
                try:
            ...
            computeTransform(audioList, options["transformType"], options["dataDirectory"],
                             options["analysisDirectory"], options)
