...
class PerceptiveGaussianLoss(Criterion):
    def __init__(self, gaussianParams, targetDims, options={'normalize': False}):
        ...
        self.latent_means = torch.from_numpy(latent_means).type('torch.FloatTensor') if issubclass(type(latent_means),
                                                                                                   np.ndarray) else latent_means.type(
            'torch.FloatTensor')
        self.latent_stds = torch.from_numpy(latent_stds).type('torch.FloatTensor') if issubclass(type(latent_stds),
                                                                                                 np.ndarray) else latent_stds.type(
            'torch.FloatTensor')
        self.targetDims = targetDims
        self.normalize = options.get('normalize', False)
