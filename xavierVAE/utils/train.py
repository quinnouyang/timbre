...
def train_model(dataset, model, loss, task=None, loss_task=None, options={}, plot_options={}, save_with={}):
    ...
    while epoch < epochs:
        ...
        with torch.no_grad():
            ...
            test_metadata = model.format_label_data(
                dataset.metadata[task][dataset.partitions['test']]) if not task is None else None
            ...
            if test_loss < min_test_loss and epoch > save_threshold:
                ...
                model.save('%s/%s_best.t7' % (results_folder, name), loss=loss, epoch=epoch,
        ...
        if epoch % save_epochs == 0:
            ...
            model.save('%s/%s_%d.t7' % (results_folder, name, epoch), loss=loss, epoch=epoch,
                       partitions=dataset.partitions, **save_with)
        ...
        if epoch % plot_epochs == 0:
            ...
            if plot_options.get('plot_reconstructions', True):
                ...
                lplt.plot_reconstructions(dataset, model, label=task,
                                          out=figures_folder + '/reconstructions_%d.svg' % epoch)
            if plot_options.get('plot_latentspace', True):
                transformation = PCA(n_components=3)
                print('plotting latent spaces...')
                lplt.plot_latent3(dataset, model, transformation, label=task, tasks=plot_tasks, layers=plot_layers,
                                  n_points=n_points, out=figures_folder + '/latent_%d' % epoch)
            if plot_options.get('plot_statistics', True):
                print('plotting latent statistics...')
                lplt.plot_latent_stats(dataset, model, label=task, tasks=plot_tasks, layers=plot_layers, legend=True,
                                       n_points=n_points, balanced=True, out=figures_folder + '/statistics_%d' % epoch)
            if plot_options.get('plot_distributions', True):
                print('plotting latent distributions...')
                lplt.plot_latent_dists(dataset, model, label=task, tasks=plot_tasks, n_points=n_points,
                                       out=figures_folder + '/dists_%d' % epoch,
                                       dims=plot_dimensions, split=False, legend=True, bins=10, relief=True)
            if plot_options.get('plot_losses', True):
                print('plotting losses...')
                lplt.plot_class_losses(dataset, model, loss, label=task, tasks=plot_tasks, loss_task=loss_task,
