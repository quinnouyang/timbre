...
class DataLoader(object):
    def __init__(self, dataset, batch_size, task=None, partition=None, *args, **kwargs):
        ...
        self.random_ids = np.split(
            random_indices[: n_batches * batch_size], len(random_indices) // batch_size
        )
        self.task = task
...
class SemiSupervisedDataLoader(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        filtered_ids = np.array(
            [x for x in filter(lambda x: not x in sup_ids, random_indices)]
        )
        self.random_ids = np.split(
            filtered_ids[: len(filtered_ids) // batch_size * batch_size],
            len(filtered_ids) // batch_size,
        )
    ...
    def __iter__(self):
        for i in range(len(self.sup_ids)):
            yield self.dataset.data[self.sup_ids[i]], self.dataset.metadata[self.task][
                self.sup_ids[i]
            ]
        for i in range(len(self.random_ids)):
