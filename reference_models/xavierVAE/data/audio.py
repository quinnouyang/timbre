...
class DatasetAudio(generic.Dataset):
    ...
    def __init__(self, options):
        ...
        self.matlabCommand = (
            options.get("matlabCommand") or "/Applications/MATLAB_R2014b.app/bin/matlab"
        )
    ...
    def computeTransforms(self, idList, options, padding=False):
        ...
        self.dataDirectory = (
            options.get("dataDirectory")
            or self.dataDirectory
            or self.dataPrefix + "/data"
        )
        self.analysisDirectory = (
            options.get("analysisDirectory")
            or self.analysisDirectory
            or self.dataPrefix + "/analysis"
        )
        transformParameters = options.get("transformParameters") or [
            self.getTransforms()[1]
        ]
        ...
        if idList is None:
            ...
            if padding:
                indices = np.pad(
                    ...
                    (
                        0,
                        self.importBatchSize - (len(self.files) % self.importBatchSize),
                    ),
                    "constant",
                    constant_values=0,
                )
    ...
    def importData(self, idList, options, padding=False):
        ...
        options["analysisDirectory"] = (
            options.get("analysisDirectory") or self.analysisDirectory
        )
    ...
    def getTransforms(self):
        ...
        transformOptions = {
            ...
            "cochleogramTC": 16,
            # Time const. (4, 16, or 64 ms), if tc == 0, the leaky integration turns to short-term avg.
            "cochleogramFac": -1,
            # Nonlinear factor (typically, .1 with [0 full compression] and [-1 half-wave rectifier]
            ...
            "strfSv": np.power(
                2, np.linspace(-2, 3, 6)
            ),  # scale vector in cyc/oct, e.g., 2.^(-2:.5:3).
...
def makeAnalysisFiles(curBatch, options):
    ...
    if options["backend"] == "matlab":
        ...
        for it in range(len(options["transformType"])):
            transformString = (
                transformString
                + "'"
                + options["transformType"][it]
                + ((it < (len(options["transformType"]) - 1)) and "'," or "'}")
            )
            finalCommand = (
                finalCommand
                + ' "transformType='
        ...
        if (options["transformOptions"]) and (
            not (options.get("defaultOptions") == True)
        ):
    ...
    for i in range(len(curBatch)):
        ...
        curAnalysisFiles[i] = (
            os.path.splitext(
                curFile.replace(
                    du.esc(options["dataDirectory"]), options["analysisDirectory"]
                )
            )[0]
            + ".npy"
        )
    ...
    if len(audioList) > 0:
        ...
        if options["backend"] == "matlab":
            ...
            curCommand = (
                ...
                + "}; cd "
                + curPath
                + '/cortical-audio; processSound; exit;"'
            )
            print(curCommand)
...
def importAudioData(curBatch, options):
    ...
    if issubclass(type(transformName), list):
        print(
            "[Warning] import transform %s among transforms %s"
            % (transformType[0], transformType)
        )
    ...
    for f in curBatch:
        if transformName == "raw":
            ...
            continue
        curAnalysisFile = re.sub(
            dataDirectory, analysisDirectory + "/" + transformName, f
        )
