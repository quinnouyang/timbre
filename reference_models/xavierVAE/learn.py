...
parser.add_argument(
    "--data", type=str, default="dataset.data", help="location of dataset.data file"
)
parser.add_argument(
    "--cuda", type=int, default=-1, help="cuda ID device (leave -1 for cpu)"
)
parser.add_argument(
    "--regularization",
    type=str,
    default="l2",
    choices=["l2", "prior", "student", "gaussian"],
    help="perceptive regularization method",
)
parser.add_argument(
    "--conditioning", type=str, default="pitch", help="conditioning task"
)
...
parser.add_argument(
    ...
    default="all",
    choices=["min", "max", "all"],
    help="regularization dimensions",
)
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="location of pre-trained model for twofold procedure",
)

parser.add_argument(
    "--latent_dims", type=int, default=32, help="number of latent dimensions"
)
parser.add_argument(
    "--hidden_dims", type=int, default=3000, help="dimension of hidden layers"
)
parser.add_argument(
...
parser.add_argument(
    "--twofold_epochs", type=int, default=2000, help="number of twofold epochs"
)
parser.add_argument("--name", type=str, default="perceptive", help="savename")

parser.add_argument(
    "--alpha", type=float, default=1.0, help="perceptive regularization weighting"
)
parser.add_argument(
    "--beta", type=float, default=1.0, help="kld regularization weighting"
)
parser.add_argument(
    "--save_epochs", type=int, default=1000, help="model save periodicity"
)
parser.add_argument(
    "--saveroot",
    type=str,
    default=CURRENT_LOCATION + "/trained_models",
...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="plot reconstructions while training",
)
parser.add_argument(
    "--plot_latentspace", type=int, default=1, help="plot latent space while training"
)
parser.add_argument(
    "--plot_statistics",
    type=int,
    default=1,
    help="plot latent statistics while training",
)
parser.add_argument(
    "--plot_npoints", type=str, default=10000, help="number of points used for plotting"
)
...
# temp = list(audioSet.data[145].flat)
# print(temp)
# rate = 22050
# scaled = np.int16(temp / np.max(np.abs(temp)) * 32767)
# write('test.wav', rate, scaled)
# winsound.PlaySound(audioSet.data[5], winsound.SND_FILENAME)

# this should filter out pitches higher than octave 5 by default.
...
# Preprocess data
# This sets the minimum amplitude value to be 1e-6, probably to prevent divide by zero or something similar
audioSet.data[audioSet.data < 1e-6] = 1e-6
# This changes the amplitude the audio to a log scale
audioSet.data = np.log(audioSet.data)
# These give information on average/min/max amplitude
...
# This seems to extract a spectral centroid, among other things parameters
timbre_path = "misc/timbre.npy"
prior_params, prior_gauss_params = percep.get_perceptual_centroids(
    audioSet, args.mds_dims, timbre_path=timbre_path
)

# Define model
# By default, ConditionalVAE is used because we only want to use a single pitch
...
if args.load is None:
    ...
    if not args.conditioning is None:
        add_params["label_params"] = {
            "dim": audioSet.classes[args.conditioning]["_length"],
    ...
    if args.cuda >= 0:
        with torch.cuda.device(args.cuda):
            prior_params = [
                torch.from_numpy(prior_params[0]).to(torch.float32).cuda(),
        ...
        prior_params = [
            torch.from_numpy(prior_params[0]).to(torch.float32),
...
plot_options = {
    "plot_reconstructions": args.plot_reconstructions,
    "plot_statistics": args.plot_statistics,
...
if args.regularization == "l2":
    perceptive_loss = PerceptiveL2Loss(
        prior_gauss_params[0], targetDims, {"normalize": normalize}
    )
elif args.regularization == "gaussian":
    perceptive_loss = PerceptiveGaussianLoss(
        prior_gauss_params, targetDims, {"normalize": normalize}
    )
elif args.regularization == "student":
    perceptive_loss = PerceptiveStudent(
        prior_gauss_params[0], targetDims, {"normalize": normalize}
    )
elif args.regularization == "prior":
    vae.platent = [
        {
            "dim": args.latent_dims,
            "dist": dist.Normal,
            "prior": pg.ClassGaussian(prior_params),
        }
    ]
...
if args.twofold:
    train_options = {
        "epochs": args.twofold_epochs,
        "save_epochs": args.save_epochs,
        "name": "%s-reg" % args.name,
        "results_folder": args.saveroot + "/" + args.name + "-reg",
    }
else:
    train_options = {
        "epochs": args.epochs,
        "save_epochs": args.save_epochs,
        "name": args.name,
        "results_folder": args.saveroot + "/" + args.name,
    }
train_model(
    audioSet,
    vae,
