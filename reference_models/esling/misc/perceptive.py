...
def get_perceptual_centroids(
    dataset,
    mds_dims,
    timbre_path="timnre.npy",
    covariance=True,
    timbreNormalize=True,
    timbreProcessing=True,
):
    if timbreProcessing == True or (
        not os.path.isfile("timbre_" + str(mds_dims) + ".npy")
    ):
        ...
        for i in range(nbIns):
            for j in range(i + 1, nbIns):
                ...
                print(
                    "%s vs. %s : mu = %.2f,  std = %.2f"
                    % (selectedInstruments[i], selectedInstruments[j], mu, std)
                )
        ...
        # Rescale means
        gaussMuRatings = (gaussMuRatings - np.min(gaussMuRatings)) / np.max(
            gaussMuRatings
        )
        # Rescale variances
        gaussStdRatings = (gaussStdRatings - np.min(gaussStdRatings)) / np.max(
            gaussStdRatings
        )
