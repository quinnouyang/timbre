    ...
    def __init__(self, fn, sr=None, chns=None, blksz=2**16, dtype=np.float32):
        ...
        if not fnd:
            ...
            if ffmpeg is not None:
                ...
                self.channels = (
                    {"mono": 1, "1 channels (FL+FR)": 1, "stereo": 2}[m.group(5)]
                    if not chns
                    else chns
                )
                dur = reduce(
                    lambda x, y: x * 60 + y, list(map(float, m.group(2).split(":")))
                )
                self.frames = int(
                    dur * self.samplerate
                ...
                def rdr():
                    while True:
                        ...
                        if len(data) == 0:
                            break
                        yield np.fromstring(data, dtype=dtype).reshape(
                            (-1, self.channels)
                        ).T
...
class SndWriter:
    def __init__(self, fn, samplerate, filefmt="wav", datafmt="pcm16", channels=1):
        ...
        self.sf = Sndfile(
            fn, mode="w", format=fmt, channels=channels, samplerate=samplerate
        )
