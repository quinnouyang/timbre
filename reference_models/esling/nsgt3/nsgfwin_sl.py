...
def nsgfwin(
    f, q, sr, Ls, sliced=True, min_win=4, Qvar=1, dowarn=True, dtype=np.float64
):
    nf = sr / 2.0
...
def nsgfwin_new(f, q, sr, Ls, sliced=True, min_win=4, Qvar=1, dowarn=True):
    ...
    if sliced:
        ...
        g, M = np.array(
            [blackharrcw(m * Qvar, csh) for m, csh in zip(M, corr_shift)]
        ).T  ####
    ...
    if sliced:
        for kk in (1, lbas + 2):
            if M[kk - 1] > M[kk]:
                g[kk - 1] = np.ones(M[kk - 1], dtype=g[kk - 1].dtype)
                g[kk - 1][
                    M[kk - 1] // 2 - M[kk] // 2 : M[kk - 1] // 2 + ceil(M[kk] / 2.0)
                ] = hannwin(M[kk])
