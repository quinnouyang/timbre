...
def blackharr(n, l=None, mod=True):
    ...
    if not mod:
        bh = (
            0.35875
            - 0.48829 * np.cos(k * (2 * pi / nn))
            + 0.14128 * np.cos(k * (4 * pi / nn))
            - 0.01168 * np.cos(k * (6 * pi / nn))
        )
    else:
        bh = (
            0.35872
            - 0.48832 * np.cos(k * (2 * pi / nn))
            + 0.14128 * np.cos(k * (4 * pi / nn))
            - 0.01168 * np.cos(k * (6 * pi / nn))
        )
...
def cont_tukey_win(n, sl_len, tr_area):
    ...
    g[
        np.logical_or(
            g < sl_len / 4.0 - tr_area / 2.0, g > 3 * sl_len / 4.0 + tr_area / 2.0
        )
    ] = 0.0
    g[
        np.logical_and(
    ...
    #
    idxs = np.logical_and(
        g >= sl_len / 4.0 - tr_area / 2.0, g <= sl_len / 4.0 + tr_area / 2.0
    )
    temp = g[idxs]
    temp -= sl_len / 4.0 + tr_area / 2.0
    temp *= pi / tr_area
    g[idxs] = np.cos(temp) * 0.5 + 0.5
    #
    idxs = np.logical_and(
        g >= 3 * sl_len / 4.0 - tr_area / 2.0, g <= 3 * sl_len / 4.0 + tr_area / 2.0
    )
    temp = g[idxs]
    temp += -3 * sl_len / 4.0 + tr_area / 2.0
    temp *= pi / tr_area
    g[idxs] = np.cos(temp) * 0.5 + 0.5
    #
    return g
