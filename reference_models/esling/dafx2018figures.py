...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="enables reconstruction plotting",
)
parser.add_argument(
    "--full_instruments",
    type=int,
    default=1,
    help="enables full instruments' sounds reconstruction",
)
parser.add_argument(
    "--pairwise_paths",
    type=int,
    default=1,
    help="enables sound sampling from an  instrument distribution to another",
)
parser.add_argument(
    "--random_paths",
    type=int,
    default=1,
    help="enables sound sampling from random paths",
)
parser.add_argument(
    "--perceptual_infer",
    type=int,
    default=1,
    help="enables perceptual inference from test dataset",
)
parser.add_argument(
    "--plot_descriptors",
    type=int,
    default=1,
    help="enables descriptors video generation",
)
parser.add_argument(
    "--descriptor_synthesis",
    type=int,
    default=1,
    help="enables descriptor-based synthesis generation",
)
parser.add_argument(
    "--labels",
    type=int,
    nargs="*",
    default=[0, 4, 6],
    help="in case of conditioning, labels passed for generation",
)
...
def regenerateAudio(
    ...
    for i in range(iterations):
        S = data * np.exp(1j * p)
        inv_p = np.array(
            list(nsgt.backward(S))
        )  # transformHandler(S, transformType, 'inverse', options)
        new_p = np.array(
            list(nsgt.forward(inv_p))
        )  # transformHandler(inv_p, transformType, 'forward', options)
...
def retrievePathFromDistribution(
    distrib, vae, cond, pca=None, scale=True, pcaD=3, latentD=curLatentDims
):
    if scale:
...
def retrieveDistributionFromPath(
    path, vae, cond, targetFreq=410, targetFrames=None, pca=None, scale=True
):
    ...
    if targetFrames is not None:
        regenDistrib = resize(
            regenDistrib, (regenDistrib.shape[0], targetFrames), mode="constant"
        )
    return regenDistrib
...
def datasetChecking(audioSet, exportName):
    ...
    plt.rcParams["animation.ffmpeg_path"] = "/usr/local/bin/ffmpeg"
    ttl = ax.text(0.4, 0.085, os.path.basename(audioSet.files[0]), va="center")
    plt.ylim((-1, 1))
    ...
    ani = animation.FuncAnimation(
        fig, updatefig, frames=audioSet.data.shape[0], interval=50, blit=True
    )
...
def plotReconstructions(
    dataset=None, vae=None, select="random", nbRows=5, nbColumns=3, figName=None
):
    ...
    for i in range(nbRows):
        for j in range(nbColumns):
            ...
            axs[i, j * 4 + 2].plot(
                scaleData(spec[i * nbColumns + j], direction="reverse"), LineWidth=2
            )
            axs[i, j * 4 + 3].plot(
                scaleData(spec_out[i * nbColumns + j], direction="reverse"),
                "r",
                LineWidth=2,
            )
            axs[i, j * 4].set_title(
                os.path.basename(dataset.files[ids[i * nbColumns + j]])
                .replace("ordinario", "")
                .replace(".wav", "")
            )
...
def plotLatentSpacePath(
    ...
    if (dataset is not None) and (vae is not None):
        ...
        Z = out["z_params_enc"][0][0].data.numpy()
        zoom = 10
    ...
    if latentPath is not None:
        ...
        for f in range(latentPath.shape[0] - 1):
            ax.plot(
                [latentPath[f, 0], latentPath[f + 1, 0]],
                [latentPath[f, 1], latentPath[f + 1, 1]],
                [latentPath[f, 2], latentPath[f + 1, 2]],
                c=cm(f / latentPath.shape[0]),
                LineWidth=2,
            )
        ax.scatter(
            latentPath[0, 0],
    ...
    if distribPlot is not None:
        for i in range(8):
            ...
            ax1.plot(
                np.linspace(0, 1, distribPlot[:, 0].shape[0]),
                distribPlot[:, int(float(i) / 8.0 * (distribPlot.shape[1] - 1))],
                c=cm(float(i) / 8.0),
            )
...
def generatePathVideo(
    ...
    (lines,) = ax1.plot(
        np.linspace(0, 1, distribPlot[:, frame].shape[0]), distribPlot[:, frame]
    )
    ax1.set_ylim([0, np.max(distribPlot)])
    ...
    Writer = animation.writers["ffmpeg"]
    writer = Writer(
        fps=float(animFrames) / (3),
        metadata=dict(artist="acids.ircam.fr"),
        bitrate=1800,
    )
    ani = animation.FuncAnimation(
        fig, updatefig, frames=animFrames, interval=50, blit=True
    )
...
def reconstructFullInstruments(
    dataset,
    vae,
    pca,
    select="loudest",
    ...
    instruNames = sorted(
        dataset.classes["instrument"], key=dataset.classes["instrument"].get
    )
    ...
    for c1 in range(11):
        ...
        metadata = vae.format_label_data(
            np.array([dataset.metadata["pitch"][curInstru]])
        )
        latentPath, pcaPath = retrievePathFromDistribution(
            curNSGT, vae, metadata, latentD=vae.platent[-1]["dim"]
        )
        if plot:
            # Plot this path
            plotLatentSpacePath(
                latentPath.T, Mp=Mp, Mc=Mc, figName=baseName + str(instruNames[c1])
            )
        # Compute distribution from this path
        latentPath = vae.format_input_data(latentPath)
        regenDistrib = retrieveDistributionFromPath(
            latentPath, vae, metadata, targetFrames=targetNSGT25
        )
        plt.figure("resynth")
        ...
        if audio:
            ...
            regenerateAudio(
                regenDistrib,
                targetLen=int(2.5 * 22050),
                curName=baseName + str(instruNames[c1]),
            )
        if video:
            ...
            generatePathVideo(
                ...
                Mp=Mp,
                Mc=Mc,
                distribPlot=regenDistrib,
                figName=baseName + str(instruNames[c1]),
            )


def reconstructPairwisePaths(
    dataset,
    vae,
    pca,
    select="note",
    ...
    for c1 in range(11):
        ...
        out = vae.forward(
            dataset.data[curInstru][np.newaxis, :], y=metadata, volatile=True
        )
        ...
        for c2 in range(11):
            ...
            if pathType in ["spherical", "expressive"]:
                ...
                omega = np.arccos(
                    np.dot(
                        curStart / np.linalg.norm(curStart),
                        curEnd / np.linalg.norm(curEnd),
                    )
                )
            for f in range(targetFrames):
                ...
                if pathType == "spherical":
                    curPoint = (
                        np.sin((1 - ratio) * omega) / np.sin(omega) * (curStart)
                    ) + (np.sin(ratio * omega) / np.sin(omega) * (curEnd))
                if pathType == "expressive":
                    curPoint = (
                        np.sin((1 - ratio) * omega)
                        / np.sin(omega)
                        * (curStart + (pathC1[ratioExpressive, :] * expressiveFactor))
                    ) + (
                        np.sin(ratio * omega)
                        / np.sin(omega)
                        * (
                            curEnd
                            + (pathC2[ratioExpressiveTarget, :] * expressiveFactor)
                        )
                    )
            ...
            # Compute distribution from this path
            regenDistrib = retrieveDistributionFromPath(
                realPoints.T, vae, metadata, targetFrames=targetFrames
            )
            # Plot this path
            if plot:
                plotLatentSpacePath(
...
def generateNDcircle(dims, center, radius, points, revolutions):
    ...
    if len(radius) > 1:
        if len(radius) == 2:
            radius = np.tile(
                np.linspace(radius[0], radius[1], points)[:, np.newaxis], (1, dims)
            )
...
def testRandomPaths(
    ...
    if randomStart:
        ...
        center = vae.forward(dataset.data[iVal][np.newaxis, :], y=metadata)[
            "z_params_enc"
        ][0][0].data.numpy()
    ...
    if curShape == "spiralOut":
        rStart = ((np.random.rand() * 0.9) + 0.1) * 0.7
        rEnd = rStart + (np.random.rand() * np.max(np.abs(Mp)))
        radius = [rStart, rEnd]
    if curShape == "spiralIn":
        rEnd = ((np.random.rand() * 0.9) + 0.1) * 0.7
    ...
    if np.random.rand(1) < 1.5:
        ...
        curCond = (
            resize(np.random.randint(0, 11, size=(4, 1)) / 11, (points, 1), mode="wrap")
            * 12
        ).astype("int")
    ...
    # Compute distribution from this path
    regenDistrib = retrieveDistributionFromPath(
        circlePath, vae, metaCond, targetFrames=targetFrames
    )
    if audio:
        # Synthesize sound from it
    ...
    if plot:
        plotLatentSpacePath(
            projPoints, Mp=Mp, Mc=Mc, distribPlot=regenDistrib, figName=curName + ".png"
        )
    # Video this path
    if video:
        generatePathVideo(
            latentPath=projPoints.T,
            Mp=Mp,
            Mc=Mc,
            distribPlot=regenDistrib,
            figName=curName,
...
def sampleCompute(model, zPoint, pca, cond, targetDims=None):
    ...
    invVal = model.decode(
        torch.Tensor(pca.inverse_transform(zPoint)[np.newaxis, :]), y=cond
    )
    invVal = scaleData(invVal[0][0][0].data.numpy(), direction="reverse").T
    invVal[invVal < 0] = 0
    # Compute all descriptors
    descValues = {
        "loudness": librosa.feature.rmse(S=invVal),
...
def sample2DSpace(vae, pca, cond, nbSamples, nbPlanes, Zp, Zc, figName=None):
    ...
    for i in range(3):
        samplingGrids[i] = np.meshgrid(
            np.linspace(-0.9, 0.9, nbSamples), np.linspace(-0.9, 0.9, nbSamples)
        )
        ...
        for plane in range(nbPlanes):
            ...
            for x in range(nbSamples):
                for y in range(nbSamples):
                    ...
                    descVals = sampleCompute(
                        vae, torch.Tensor(curPoint), pca, cond, targetDims=[0, 1, 2]
                    )
                    for d in descriptors:
...
def sampleBatchCompute(model, zs, pca, cond, targetDims=None):
    ...
    for i in range(invVal.shape[0]):
        ...
        descValues["centroid"][i] = librosa.feature.spectral_centroid(S=currentFrame)[
            0
        ][0]
        descValues["flatness"][i] = librosa.feature.spectral_flatness(S=currentFrame)[
            0
        ][0]
        descValues["bandwidth"][i] = librosa.feature.spectral_bandwidth(S=currentFrame)[
            0
        ][0]
        descValues["rolloff"][i] = librosa.feature.spectral_rolloff(S=currentFrame)[0][
            0
        ]
    return descValues
    ...
    for x in range(sampleGrid3D[0].shape[0]):
        for y in range(sampleGrid3D[0].shape[1]):
            for z in range(sampleGrid3D[0].shape[2]):
                curPoint = [
                    sampleGrid3D[0][x, y, z],
                    sampleGrid3D[1][x, y, z],
                    sampleGrid3D[2][x, y, z],
                ]
...
def sample3DSpace(
    ...
    samplingGrid3D = np.meshgrid(
        np.linspace(np.min(Zp[:, 0]), np.max(Zp[:, 0]), nbSamples),
    ...
    for dim in range(3):
        ...
        for d in descriptors:
            ...
            if dim == 0:
                surfYZ = np.array(
                    [
                        [xVals[0], np.min(Zp[:, 1]), np.min(Zp[:, 2])],
                        [xVals[0], np.max(Zp[:, 1]), np.min(Zp[:, 2])],
                        [xVals[0], np.max(Zp[:, 1]), np.max(Zp[:, 2])],
                        [xVals[0], np.min(Zp[:, 1]), np.max(Zp[:, 2])],
                    ]
                )
            if dim == 1:
                surfYZ = np.array(
                    [
                        [np.min(Zp[:, 0]), yVals[0], np.min(Zp[:, 2])],
                        [np.max(Zp[:, 0]), yVals[0], np.min(Zp[:, 2])],
                        [np.max(Zp[:, 0]), yVals[0], np.max(Zp[:, 2])],
                        [np.min(Zp[:, 0]), yVals[0], np.max(Zp[:, 2])],
                    ]
                )
            if dim == 2:
                surfYZ = np.array(
                    [
                        [np.min(Zp[:, 0]), np.min(Zp[:, 1]), zVals[0]],
                        [np.max(Zp[:, 0]), np.min(Zp[:, 1]), zVals[0]],
                        [np.max(Zp[:, 0]), np.max(Zp[:, 1]), zVals[0]],
                        [np.min(Zp[:, 0]), np.max(Zp[:, 1]), zVals[0]],
                    ]
                )
            ...
            for j in range(4):
                (lines[j],) = ax.plot(
                    [surfYZ[j, 0], surfYZ[(j + 1) % 4, 0]],
                    [surfYZ[j, 1], surfYZ[(j + 1) % 4, 1]],
                    zs=[surfYZ[j, 2], surfYZ[(j + 1) % 4, 2]],
                    linestyle="--",
                    color="k",
                    linewidth=2,
                )
            for v in range(nbSamples):
                if dim == 0:
                    surfYZ = np.array(
                        [
                            [xVals[v], np.min(Zp[:, 1]), np.min(Zp[:, 2])],
                            [xVals[v], np.max(Zp[:, 1]), np.min(Zp[:, 2])],
                ...
                if dim == 1:
                    surfYZ = np.array(
                        [
                            [np.min(Zp[:, 0]), yVals[v], np.min(Zp[:, 2])],
                            [np.max(Zp[:, 0]), yVals[v], np.min(Zp[:, 2])],
                ...
                if dim == 2:
                    surfYZ = np.array(
                        [
                            [np.min(Zp[:, 0]), np.min(Zp[:, 1]), zVals[v]],
                            [np.max(Zp[:, 0]), np.min(Zp[:, 1]), zVals[v]],
            ...
            def updatefig(*args):
                ...
                try:
                    ...
                    if dim == 0:
                        surfYZ = np.array(
                            [
                                [xVals[i], np.min(Zp[:, 1]), np.min(Zp[:, 2])],
                    ...
                    if dim == 1:
                        surfYZ = np.array(
                            [
                                [np.min(Zp[:, 0]), yVals[i], np.min(Zp[:, 2])],
                    ...
                    if dim == 2:
                        surfYZ = np.array(
                            [
                                [np.min(Zp[:, 0]), np.min(Zp[:, 1]), zVals[i]],
            ...
            Writer = animation.writers["ffmpeg"]
            writer = Writer(
                fps=15, metadata=dict(artist="acids.ircam.fr"), bitrate=1800
            )
            ani = animation.FuncAnimation(
                fig, updatefig, frames=nbSamples, interval=50, blit=True
            )
...
def descriptorBasedSynthesis(
    ...
    pdb.set_trace()
    curPoint = [
        int(np.round((instruPos[0] + 3) * 5)),
        int(np.round((instruPos[1] + 3) * 5)),
        int(np.round((instruPos[1] + 2) * 5)),
    ]
    #    curPoint = [3,2,1]
    ...
    tmpPoint[:3] = [
        samplingGrid3D[0][curPoint[0], curPoint[1], curPoint[2]],
        samplingGrid3D[1][curPoint[0], curPoint[1], curPoint[2]],
        samplingGrid3D[2][curPoint[0], curPoint[1], curPoint[2]],
    ]
    ...
    for p in range(1, nbPoints):
        ...
        curNeighborhood = descVals[curPoint[0] - 1 : curPoint[0] + 2, :, :][
            :, curPoint[1] - 1 : curPoint[1] + 2, :
        ][:, :, curPoint[2] - 1 : curPoint[2] + 2].copy()
        # Now differentiate with current point
        curNeighborhood -= descVals[curPoint[0], curPoint[1], curPoint[2]]
        curNeighborhood[1, 1, 1] = 1e9
        # Check next point
        nextPoint = np.unravel_index(
            np.argmin(np.abs(curNeighborhood - curDiff)), curNeighborhood.shape
        )
        # Update point
    ...
    for f in range(nbPoints * 20):
        ...
        tmpPoint = ((1 - ratio) * fullRealPath[idP]) + ratio * fullRealPath[
            min(idP + 1, nbPoints - 1)
        ]
        upsamplePath[f, :] = tmpPoint
    # Compute distribution from this path
    regenDistrib = retrieveDistributionFromPath(
        upsamplePath, vae, cond, targetFrames=targetFrames
    )
    if audio:
        # Synthesize sound from it
...
def descriptorBasedTest(
    ...
    samplingGrid3D = np.meshgrid(
        np.linspace(
            np.min(Zp[:, 0]), np.max(Zp[:, 0]), descriptorSpace["centroid"].shape[0]
        ),
        np.linspace(
            np.min(Zp[:, 1]), np.max(Zp[:, 1]), descriptorSpace["centroid"].shape[0]
        ),
        np.linspace(
            np.min(Zp[:, 2]), np.max(Zp[:, 2]), descriptorSpace["centroid"].shape[0]
        ),
    )
    ...
    for desc in ["centroid", "flatness", "bandwidth"]:
        ...
        for repeat in range(nbRepeat):
            ...
            metadata = vae.format_label_data(
                np.array([dataset.metadata["pitch"][sInstru]])
            )
            ...
            for cIDtest in range(3):
                ...
                curName = (
                    baseName
                    + os.path.basename(dataset.files[sInstru])[:-4]
                    + "_"
                    + desc
                    + "_"
                    + curveNames[cID]
                )
                descriptorBasedSynthesis(
                    dataset,
                    vae,
                    cond,
                    pca,
...
def projectDataset(setRoot, vae, pca, Zp, Zc, Mp, Mc, figName=None):
    ...
    ax.scatter(
        ...
        Ztest[:, 1],
        Ztest[:, 2],
        c=cmap_test(testSet.metadata.get("instrument")),
        s=10,
    )
    ...
    for repeat in range(3):
        plotReconstructions(
            vae=vae,
            dataset=testSet,
            figName=figName + "reconstructions_" + str(repeat) + ".png",
        )
...
for mID in range(len(models)):
    ...
    pca, Zp, Zc, Mp, Mc = plotLatentSpacePath(
        vae=vae, dataset=audioSet, figName=curDir + "spacePCA.png"
    )
    for r in range(2):
        if args.plot_reconstructions == 1:
            ...
            plotReconstructions(
                vae=vae,
                dataset=audioSet,
                figName=curDir + "reconstructions_" + str(r) + ".png",
            )
        if args.full_instruments == 1:
            print("  - Reconstruct full instruments")
            reconstructFullInstruments(
                audioSet,
                vae,
                baseName=curDir + "recons_full_%d/" % r,
                Zp=Zp,
                Zc=Zc,
                Mp=Mp,
                Mc=Mc,
    ...
    if args.pairwise_paths == 1:
        ...
        reconstructPairwisePaths(
            audioSet,
            vae,
            pca,
            pathType="spherical",
            targetFrames=targetNSGT25,
            baseName=curDir + "recons_pairwise/",
            Zp=Zp,
            Zc=Zc,
            Mp=Mp,
            Mc=Mc,
            ...
            for shape in ["spiralOut", "spiralIn"]:
                for start in [True, False]:
                    testRandomPaths(
                        ...
                        vae,
                        pca,
                        targetFrames=targetNSGT5,
                        plot=True,
                        baseName=curDir + "random_paths/",
                        curShape=shape,
                        randomStart=start,
                        Zp=Zp,
                        Zc=Zc,
    ...
    # Compute a full PCA
    if (
        args.perceputal_infer == 1
        or args.plot_descriptor == 1
        or args.descriptor_synthesis == 1
    ):
        Z = vae.forward(
            vae.format_input_data(audioSet.data),
            y=vae.format_label_data(audioSet.metadata["pitch"]),
        )["z_params_enc"][0][0].data.numpy()
        pca_f = PCA(n_components=3)
        Zp_f = pca_f.fit_transform(Z)
        if args.perceputal_infer == 1:
            print("  - Projecting a new dataset")
            projectDataset(
                "/Users/chemla/Datasets/acidsInstruments-test",
                vae,
                pca,
                Zp,
                Zc,
                Mp,
                Mc,
                figName=curDir + "novel_projection/",
            )
        ...
        if args.plot_descriptors == 1 or args.descriptor_synthesis == 1:
            ...
            for curCond in args.labels:
                ...
                resultTensor = sample3DSpace(
                    vae,
                    pca_f,
                    metaCond,
                    args.descriptors_steps,
                    10,
                    Zp,
                    Zc,
                    loadFrom=loadFrom,
                    saveAs=saveAs,
                    figName=curDir + "descriptor_space/space3D_c" + str(curCond),
                )
                ...
                if args.descriptor_synthesis == 1:
                    ...
                    descriptorBasedTest(
                        ...
                        select="loudest",
                        nbPoints=20,
                        descriptorSpace=resultTensor,
                        baseName=curDir + "descriptor_synthesis/",
                        Mp=Mp,
                        Mc=Mc,
                    )
