...
def train_model(
    ...
    while epoch < epochs:
        ...
        epoch += 1
        loader = DataLoader(
            dataset, batch_size=batch_size, partition="train", task=task
        )
        ...
        with torch.no_grad():
            ...
            test_metadata = (
                model.format_label_data(
                    dataset.metadata[task][dataset.partitions["test"]]
                )
                if not task is None
                else None
            )
            ...
            if loss_task == task:
                ...
                if not loss_task is None:
                    ...
                    y_task = model.format_label_data(
                        dataset.metadata.get(loss_task)[test_ids], plabel=plabel
                    )
                else:
                    y_task = None
            ...
            if test_loss < min_test_loss and epoch > save_threshold:
                ...
                print(
                    "-- saving best model at %s"
                    % "results/%s/%s_%d.t7"
                    % (results_folder, name, epoch)
                )
                model.save(
                    "%s/%s_best.t7" % (results_folder, name),
                    loss=loss,
                    epoch=epoch,
        ...
        if epoch % save_epochs == 0:
            ...
            model.save(
                "%s/%s_%d.t7" % (results_folder, name, epoch),
                loss=loss,
                epoch=epoch,
                partitions=dataset.partitions,
                **save_with
        ...
        if epoch % plot_epochs == 0:
            plt.close("all")
            n_points = plot_options.get(
                "plot_npoints", min(dataset.data.shape[0], 5000)
            )
            ...
            plot_layers = plot_options.get(
                "plot_layers", list(range(len(model.platent)))
            )
            if plot_options.get("plot_reconstructions", True):
                print("plotting reconstructions...")
                lplt.plot_reconstructions(
                    dataset,
                    model,
                    label=task,
        ...
        if image_export:
            ...
            out = model.pinput[0]["dist"](
                *model.forward(images, y=metadata)["x_params"][0]
            ).mean
            torchvision.utils.save_image(
                out.reshape(out.size(0), 1, 28, 28),
                figures_folder + "grid_%d.png" % epoch,
                nrow=nb_reconstructions,
            )

    model.save(
        "%s/%s_final.t7" % (results_folder, name),
        loss=loss,
        epoch=epoch,
        partitions=dataset.partitions,
        **save_with
