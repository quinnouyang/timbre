...
def check_data(dataset, out="", tasks=None):
    ...
    for t, task in enumerate(tasks):
        all_ids, classes = (
            get_class_ids(dataset, task)
            if task_relative
            else ([list(range(dataset.data[0]))], None)
        )
        ...
        for n, ids in enumerate(all_ids):
            ...
            ttl = ax.text(
                0.4, 0.085, os.path.basename(dataset.files[ids[0]]), va="center"
            )
            plt.ylim((-1, 1))
            ...
            ani = animation.FuncAnimation(
                fig, updatefig, frames=len(ids) - 2, interval=50, blit=True
            )
            title = (
                out + "/datasetCheck_%s_%s.mp4" % (task, class_names[n])
                if task_relative
                else out + "/datasetCheck.mp4"
            )
