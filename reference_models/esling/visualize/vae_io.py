...
def make_confusion_matrix(out, dataset, task):
    cnf_matrix = confusion_matrix(
        fromOneHot(out["y"].data.numpy()), fromOneHot(dataset.metadata[task])
    )
...
def make_grid(images, shape, original_shape):
    if images.shape[0] != shape[0] * shape[1]:
        raise Exception(
            "Number of images %d does not match the given shape : %s"
            % (images.shape[0], shape)
        )
    ...
    for i in range(shape[0]):
        for j in range(shape[1]):
            grid[
                i * original_shape[0] : (i + 1) * original_shape[0],
                j * original_shape[1] : (j + 1) * original_shape[1],
            ] = np.reshape(images[idx, :], original_shape)
            idx += 1
...
def export_pairs_img(path, dataset, model, ids=None, task=None, original_size=None):
    ...
    if original_size is None:
        raise Exception(
            "original size of image not found in arguments nor dataset attributes."
        )
    if not issubclass(type(ids), list) and type(ids) != np.ndarray:
        if type(ids) == int or type(ids) == float:
            ...
            raise Exception(
                "ids key argument seems to be uncorrect. Please give an array of ids or a number of images to draw."
            )
...
def export_grid_img(
    ...
    if latent_dim > 2:
        if manifold_name is None:
            raise Exception(
                "latent space dimension is greater than 2 ; please provide a adequate manifold"
            )
        try:
            manifold = model.manifolds[manifold_name]
        except:
            raise Exception(
                "model does not seem to possess the manifold %s. Please give a valid manifold name"
                % manifold
            )
        print(
            "[export_img_grid]-- dim(z) > 2. processing to dimension reduction with manifold %s..."
            % manifold_name
        )
...
def plot_reconstructions(model, dataset, n_samples=5, ids=None):
    if ids is None:
        n_batches = (
            ...
            if not issubclass(type(dataset.data[0]), list)
            else dataset.data[0].shape[0]
        )
