...
class GaussianLayer(nn.Module):
    ...
    def __init__(self, pinput, poutput):
        ...
        var_module = nn.Sequential(
            nn.Linear(input_dim_var, self.output_dim), nn.Sigmoid()
        )
...
class AutoRegressiveGaussianLayer(GaussianLayer):
    def __init__(self, pinput, poutput):
        ...
        autoregressive_module = nn.Sequential(
...
class BernoulliLayer(nn.Module):
    ...
    def __init__(self, pinput, poutput):
        ...
        self.input_dim = pinput["dim"]
        self.output_dim = poutput["dim"]
        self.modules_list = nn.Sequential(
            nn.Linear(self.input_dim, self.output_dim), nn.Sigmoid()
        )
...
class CategoricalLayer(nn.Module):
    ...
    def __init__(self, pinput, plabel):
        ...
        self.input_dim = pinput["dim"]
        self.label_dim = plabel["dim"]
        self.modules_list = nn.Sequential(
            nn.Linear(self.input_dim, self.label_dim), nn.Softmax()
        )
