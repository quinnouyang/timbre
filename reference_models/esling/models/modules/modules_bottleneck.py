...
class MLP(nn.Module):
    ...
    def get_hidden_layers(
        cls,
        pins,
        phidden={"dim": 800, "nlayers": 2, "batch_norm": False},
        nn_lin="ReLU",
        name="",
    ):
...
class DLGMLayer(nn.Module):
    ...
    def __init__(
        self, pins, pouts, phidden={"dim": 800, "nlayers": 2}, nn_lin="ReLU", name=""
    ):
        """
        ...
        self.hidden_module = MLP.get_hidden_layers(
            pins, phidden=phidden, nn_lin=nn_lin, name=name
        )
        if issubclass(type(pouts), list):
            ...
            for pout in pouts:
                ...
                self.cov_module.append(
                    nn.Sequential(nn.Linear(pout["dim"], pout["dim"]), nn.Sigmoid())
                )
                init_module(self.cov_module, "Sigmoid")
        else:
            self.out_module = nn.Linear(phidden["dim"], pouts["dim"])
            init_module(self.out_module, "Linear")
            self.cov_module = nn.Sequential(
                nn.Linear(pouts["dim"], pouts["dim"]), nn.Sigmoid()
            )
            init_module(self.cov_module, "Sigmoid")
...
class DiscriminatorLayer(nn.Module):
    def __init__(
        self, pins, phidden={"dim": 800, "nlayers": 2}, name="", nn_lin="ReLU"
    ):
        super(DiscriminatorLayer, self).__init__()
        if not issubclass(type(pins), list):
            pins = [pins]
        self.hidden_module = nn.Sequential(
            MLP(pins, phidden=phidden, pouts=None, nn_lin=nn_lin),
            nn.Linear(phidden["dim"], 1),
            nn.Sigmoid(),
        )
