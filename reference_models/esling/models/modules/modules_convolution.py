...
class ConvLayer(nn.Module):
    ...
    def init_pooling(self, dim, kernel_size, return_indices):
        self.add_module(
            "pool_module",
            self.pool_modules[dim](kernel_size, return_indices=self.return_indices),
        )
...
class Convolutional(nn.Module):
    ...
    def get_output_conv_length(self):
        ...
        for l in range(self.depth):
            ...
            current_output = (
                current_output
                + 2 * np.array(current_module.padding)
                - np.array(current_module.dilation)
                * (np.array(current_module.kernel_size) - 1)
                - 1
            )
            current_output = np.floor(
                current_output / np.array(current_module.stride) + 1
            )
            # pooling layers
...
class ConvolutionalLatent(Convolutional):
    def __init__(
        ...
        pins,
        pouts,
        phidden={"dim": [64, 32, 8], "conv_dim": 2, "kernel_size": [16, 8, 4]},
        *args,
        **kwargs
    ):
        # input parameters
        ...
        super(ConvolutionalLatent, self).__init__(
            pins, pouts, phidden=phidden, *args, **kwargs
        )
        ...
        self.post_encoder = self.get_post_mlp(
            mlp_input_size * self.n_channels[-1], self.pouts, *args, **kwargs
        )
...
class DeconvolutionalLatent(Convolutional):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(DeconvolutionalLatent, self).__init__(
            pouts, pins, nn_lin=nn_lin, phidden=phidden, *args, **kwargs
        )
        ...
        if encoder is None or not encoder.return_indices:
            raise Warning(
                "Deconvolutional module has to be initialized with a valid Convolutional module in order to perform appropriate Unpooling."
            )
        mlp_output_size = int(
            reduce(lambda x, y: x * y, self.output_size) * phidden["dim"][0]
        )
