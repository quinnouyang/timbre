...
def kld_gaussian_gaussian(gauss1, prior=[], logvar=False, size_average=False):
    ...
    if prior == []:
        mean2, std2 = (
            torch.zeros_like(gauss1[0], device=mean1.device),
            torch.zeros_like(gauss1[1], device=std1.device),
        )
    else:
    ...
    result = 0.5 * (
        ...
        - std1
        + torch.exp(std1 - std2)
        + torch.pow(mean1 - mean2, 2) / torch.exp(std2)
        - 1
    )
...
def kld(dist1, dist2):
    if dist1 == dist2 == dist.Normal:
        ...
        raise Exception(
            "Don't find KLD module for distributions %s and %s" % (dist1, dist2)
        )
