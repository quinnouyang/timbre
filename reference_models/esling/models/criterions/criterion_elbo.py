...
class ELBO(Criterion):
    ...
    def get_reconstruction_error(self, model, x, out, *args, **kwargs):
        ...
        for i in range(len(x)):
            rec_error = rec_error + log_density(model.pinput[i]["dist"])(
                x[i], out["x_params"][i], size_average=self.size_average
            )
        return rec_error
        ...
        for l, platent_tmp in enumerate(model.platent):
            ...
            for i, pl in enumerate(platent_tmp):
                if l == len(model.platent) - 1:
                    ...
                    kld_error = kld_error + kld(pl["dist"], prior.dist)(
                        current_zenc_params[i],
                        prior.get_params(device=model.device, *args, **kwargs),
                        size_average=self.size_average,
                    )
                else:
                    current_zdec_params = checklist(out["z_params_dec"][l])
                    kld_error = kld_error + kld(pl["dist"], pl["dist"])(
                        current_zenc_params[i],
                        current_zdec_params[i],
                        size_average=self.size_average,
                    )
        return kld_error
    ...
    def get_montecarlo(self, model, out, *args, **kwargs):
        ...
        for l, platent_tmp in enumerate(model.platent):
            ...
            for i, pl in enumerate(platent_tmp):
                ...
                if l == len(model.platent) - 1:
                    prior = pl.get("prior") or IsotropicGaussian(pl["dim"])
                    log_p = log_density(prior.dist)(
                        current_zenc[i],
                        prior.get_params(device=model.device, *args, **kwargs),
                    )
                else:
                    current_zdec_params = checklist(out["z_params_dec"][l])
                    log_p = log_density(pl["dist"])(
                        current_zenc[i],
                        current_zdec_params[i],
                        size_average=self.size_average,
                    )
                # get q(z | z_params)
                log_q = log_density(pl["dist"])(
                    current_zenc[i],
                    current_zenc_params[l],
                    size_average=self.size_average,
                )
    ...
    def loss(
        self, model, out, epoch=None, x=None, options={}, write=None, *args, **kwargs
    ):
