...
class VanillaDLGM(VanillaVAE):
    def __init__(
        ...
        **kwargs
        ...
        super(VanillaDLGM, self).__init__(
    ...
    def make_decoders(
        ...
        **kwargs
        ...
        top_dim = (
            latent_params[-1]["dim"]
            if not issubclass(type(latent_params[-1]), list)
            else sum([x["dim"] for x in latent_params[-1]])
        )
        decoders.append(nn.Linear(top_dim, top_dim))
        for i in reversed(range(0, len(latent_params))):
            if i == 0:
                phidden_dec = dict(hidden_params[0])
                phidden_dec["batch_norm"] = False
    ...
    def decode(
        self, z_enc=[], y=None, options={"sample": True}, decoders=None, *args, **kwargs
    ):
        sample = options.get("sample") or True
        if decoders is None:
        ...
        for layer in range(1, len(self.platent)):
            try:
                ...
                current_z_enc = Tensor(
                    z_enc[-1].size(0),
                    self.platent[-layer - 1]["dim"],
                    requires_grad=True,
                ).zero_()
                if self.is_cuda:
