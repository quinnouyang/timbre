...
class VanillaVAE(AbstractVAE):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(VanillaVAE, self).__init__(
            input_params, latent_params, hidden_params, *args, **kwargs
        )
        ...
        **kwargs
        ...
        for layer in reversed(range(len(latent_params))):
            if layer == 0:
                ...
                new_decoder = self.make_decoder(
                    input_params,
                    latent_params[0],
                    hidden_params[0],
                    name="vae_decoder_%d" % layer,
                    encoder=self.encoders[layer],
                )
            else:
                new_decoder = self.make_decoder(
                    latent_params[layer - 1],
    ...
    def make_decoder(cls, input_params, latent_params, hidden_params, *args, **kwargs):
        ...
        ModuleClass = hidden_params.get("class", MLP)
        module = hidden_params.get("shared_decoder") or ModuleClass(
            latent_params, input_params, hidden_params, *args, **kwargs
        )
        module = ModuleClass(
            latent_params, input_params, hidden_params, *args, **kwargs
        )
        ...
        for layer in range(len(self.platent) + 1):
            if layer == 0:
                ...
                if not issubclass(type(self.platent[layer - 1]), list):
                    if sample:
                        ins = self.platent[layer - 1]["dist"](
                            *z_params[layer - 1]
                        ).rsample()
                    else:
                        ins = (
                            self.platent[layer - 1]["dist"]
                            .analytic_mean(*z_params[layer - 1], **kwargs)
                            .rsample()
                        )
                else:
                    ins = []
                    for i, latent_group in enumerate(self.platent[layer - 1]):
                        if sample:
                            z_tmp = latent_group["dist"](
                                *z_params[layer - 1][i]
                            ).rsample()
                        else:
                            z_tmp = (
                                latent_group["dist"]
                                .analytic_mean(*z_params[layer - 1][i])
                                .rsample()
                            )
                        ins.append(z_tmp)
                z.append(ins)
    ...
    def decode(
        self, z_in=[], y=None, sample=True, path="encoder", layer=-1, *args, **kwargs
    ):
        assert layer != 0
    ...
    def forward(self, x, options={}, *args, **kwargs):
        ...
        return {
            "x_params": x_params,
            "z_params_dec": z_params_dec,
            "z_dec": z_dec,
            "z_params_enc": z_params_enc,
            "z_enc": z_enc,
        }
    ...
    def init_optimizer(self, optim_params):
        ...
        self.optimizers = {
            "default": getattr(torch.optim, alg)(self.parameters(), **optim_args)
        }
        scheduler = optim_params.get("scheduler", "ReduceLROnPlateau")
        scheduler_args = optim_params.get(
            "scheduler_args", {"patience": 100, "factor": 0.2, "eps": 1e-10}
        )
        self.schedulers = {
            "default": getattr(torch.optim.lr_scheduler, scheduler)(
                self.optimizers["default"], **scheduler_args
            )
        }
